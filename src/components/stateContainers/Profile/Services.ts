import { SubscriptionServices, UpdateProfileservice ,UpdatePlayerservice} from "../../../utils/API";
...
export const updatePlayerService = async (item: any): Promise<any> => {
  return UpdatePlayerservice.updatePlayerProfile(item)
    .then((res: any) => res.status)
    .catch((error: any) => error);
};
